...
export class StaticEntity extends IEntityDSG {
  static n = 0
  ...
  prepareToRender(renderInstManager: GfxRenderInstManager) {
    ...
    let offs = template.allocateUniformBuffer(Program.ub_ModelParams, 18)
    const mapped = template.mapUniformBufferF32(Program.ub_ModelParams)

    offs += fillMatrix4x4(mapped, offs, this.matrix)
    mapped[offs++] = this.textureFound ? 1. : 0.
    mapped[offs++] = this.alphaTest ? 1. : 0.
