...
import { CreateSingletons, WorldScene } from './world.js'
...
class SceneDesc implements Viewer.SceneDesc {
    ...
    public async createScene(gfxDevice: GfxDevice, context: SceneContext): Promise<Viewer.SceneGfx> {
        ...
        for (let i = 0; i < this.paths.length; i++) {
            let sector_buffer = await dataFetcher.fetchData(`shar/art/${this.paths[i]}`)
            h.load(world_scene, sector_buffer)
        }
        return new Bug(world_scene, gfxDevice)
    }
}
